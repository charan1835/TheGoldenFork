...
export default function FlowingMenu({ items = [] }) {
  ...
  const isTouchDevice = typeof window !== 'undefined' && matchMedia && matchMedia('(pointer: coarse)').matches;

  return (
    <div ref={containerRef} onMouseMove={isTouchDevice ? undefined : handleMouseMove} className="relative mx-auto max-w-6xl py-2 sm:py-4">
      <div className="absolute inset-0 -z-10 bg-gradient-to-b from-background via-background/60 to-transparent" />

      {/* Mobile: horizontal scroll with snap */}
      <ul className="flex sm:hidden items-stretch gap-3 overflow-x-auto snap-x snap-mandatory px-4 no-scrollbar">
        {items.map((item, idx) => (
          <li
            key={idx}
            className="snap-center relative h-48 w-[72vw] max-w-[320px] flex-shrink-0 cursor-pointer select-none overflow-hidden rounded-2xl border border-border/60 bg-surface shadow-soft"
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/30 px-3 py-1 text-xs font-semibold text-white backdrop-blur">
                {item.text}
              </div>
            </div>
          </li>
      ...
      {/* Desktop: centered, subtle float scaling */}
      <div className="hidden sm:block" style={{ height: '320px' }}>
        <ul className="flex h-full items-center justify-center gap-4 sm:gap-6">
          {items.map((item, idx) => {
            const cardCenter = (idx + 0.5) * 240;
